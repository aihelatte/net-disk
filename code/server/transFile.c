...
// 利用TLV协议，先传输文件大小，固定的int类型，4个字节，再传输文件内容
...
int Upload(int netFd, char *file, int user_id)
{
    ...
    int code;
    get_user_code(user_id, &code);      //得到用户所在当前文件夹的code
    char md5_str[MD5_STR_LEN + 1] = {0};
    
    //找到对应文件的md5值
    get_virtual_file_md5(file, user_id, code, md5_str);
    ...
    printf("exitSize = %d\n", exitSize);
    printf("fileSize = %d\n", fileSize);
    if(fileSize >= 1073741824)// 大于1G，基于mmap的零拷贝
    {
        //直接将数据从文件对象的缓冲区发送到netfd，不适用断点续传
    ...
    {
        // 小于1G用小火车分次传输
        while (total < fileSize)        //从total开始传，可能有断点续传
        {
            if (fileSize - total < sizeof(train.buf))//最后一次发送，发送剩余内容
            {
...
int Download(int sockFd, int user_id)
{
    ...
    // 下载文件
    // 判断文件大小，是否一次接收
    if(fileSize >= 1073741824)// 大于1G，进行零拷贝
    {
